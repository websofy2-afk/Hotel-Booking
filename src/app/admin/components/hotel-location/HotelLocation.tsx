...
import { HotelLocationModel } from "./HotelLocationModel";
...
export const HotelLocation = () => {
    ...
    const fetchData = async () => {
        const res = await fetch("/api/auth/hotel-location");
        const hotelLocationJson = await res.json();
        setData(hotelLocationJson.data);
    };
    ...
    const filteredData = data.filter(
        (item) =>
            item.hotelLocation.toLowerCase().includes(search.title.toLowerCase())
    );
    ...
    const handleSave = async (form: any) => {
        ...
        try {
            if (modal.mode === "create") {
                res = await fetch("/api/auth/hotel-location", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(form)
                });
            } else if (modal.mode === "edit") {
                res = await fetch(`/api/auth/hotel-location/${modal.item._id}`, {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify(form),
                });
    ...
    return (
        <div className="p-4">
            ...
            <PageBreadcrumb pageTitle="Hotel Location" />
            ...
            <table className="min-w-full bg-white rounded shadow">
                <thead>
                    <tr className="bg-herobg">
                        {["Location", "Focus On", "Actions"].map((item, index) => (
                ...
                <tbody>
                    {currentData.map((item) => (
                        <tr key={item._id} className="text-center">
                            ...
                            <td className="border border-border shadow-md text-start shadow-skyBlue/20 p-2">{item.focusOn}</td>
