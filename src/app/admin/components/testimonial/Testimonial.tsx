...
export const Testimonial = () => {
    ...
    const handleOnChange = (name: string, review: string, rating: number) => {
        setUserReview({ ...userReview, fullName: name, review: review, rating: rating });
    }
    ...
    const fetchData = async () => {
        ...
        const testimonialJson = await res.json();
        setData(testimonialJson.data);
    };
    ...
    const filteredData = data.filter(
        (item) =>
            item.fullName.toLowerCase().includes(search.title.toLowerCase())
    );
    ...
    return (
        <div className="p-4">
            ...
            <table className="min-w-full bg-white rounded shadow">
                <thead>
                    <tr className="bg-herobg">
                        {["Name", "Reveiw", "Actions"].map((item, index) => (
                ...
                <tbody>
                    {currentData.map((item) => (
                        <tr key={item._id} className="text-center">
                            <td className="border border-border shadow-md shadow-skyBlue/20 p-2">{item.fullName}</td>
                            <td className="border border-border shadow-md shadow-skyBlue/20 p-2
                            items-center justify-center text-justify flex gap-1
                            ">
                                <span className="w-[21em]">{item.review.substring(0, 40)}...</span>
                                <span onClick={() => {
                                    handleOnChange(item.fullName, item.review, item.rating);
            ...
            {
                showMessage && <MessageModel closedModel={setShowMessage} data={userReview} mode="review" />
            }
        </div>
    );
}
