...
import { hotelData } from "@/utils/constant";
import Image from "next/image";
import Link from "next/link";

const PropertyCard = ({ location }: { location: any }) => {
  const { city, focusOn, whyChooseUs, rating, featureImage, features, viewMode } = location
  const noOfHotels = location ? hotelData[city].hotels.length : null;
          ...
          >
            <div className="flex flex-col gap-1 border-b border-border mb-6">
              ...
              <div className="pb-4 space-y-3">
                ...
                <p className="text-sm bg-[#DAE7FF] text-midnight_text text-opacity-80 py-1 px-2 rounded-md ">
                  {focusOn}
                </p>
              </div>
            </div>
            ...
            <div className="h-48">
              {
                features.map((item: string, index: number) => (
                  ...
                  <div key={index} className="flex items-center gap-3 mt-2">
                    ...
                    <p className="text-midnight_text text-opacity-80 text-[15px]">{item}</p>
                  </div>
                ))
              }
            </div>
            <div>
              ...
              <h3 className="text-midnight_text font-semibold text-justify text-opacity-80 text-[15px] h-16 mb-2 before:content-['“'] after:content-['”']">{whyChooseUs}</h3>
            </div>
            <button
